...
\begin{frame}{Qu'est-ce qu'une interruption?}
  ...
  \item Les interrruption matérielles:
    ...
    \item  \textbf{Exception}.   Asynchrone  ou Synchrone.   Exemples:
      ...
      faute,  etc...  \note{Une overflow  arithmétique ne  produit pas
        d'exception, il lève le flags ``retenue''} \note{Un breakpoint
        logiciel   change  une   instruction   par  une   interruption
        logicielle. Un break point  software n'est pas possible en ROM
        alors que le breakpoint hardware oui}
    \end{itemize} 
  \item \textbf{Logicielle}. Déclenché par une instruction. Synchrone.
  \end{itemize}
\end{frame} 
